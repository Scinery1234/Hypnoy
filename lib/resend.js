...
export async function sendConfirmation({ booking, client, zoomLink, sessionsTotal = 1, paymentPlan = false, instalmentCents = 0, totalCents = 0, recurDates = null }) {
  ...
  const html = emailWrapper({
    ...
    footer: `
      ...
      <p style="font-size:12px;color:#aaa;margin-top:8px">An Outlook / Apple Calendar invite is attached to this email.</p>
  ...
  await Promise.all([
    resend.emails.send({
      ...
      subject,
      html,
      attachments: [{ filename: 'vikashan-session.ics', content: Buffer.from(ics).toString('base64'), type: 'text/calendar; method=REQUEST' }]
    }),
    resend.emails.send({
      from: `${process.env.FROM_NAME} <${process.env.FROM_EMAIL}>`,
